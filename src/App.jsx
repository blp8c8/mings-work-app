...
const TAKING_FIELDS = [
  ...
  {key:"depositReceipt",label:"Deposit Receipt",sign:1,hasCashCard:true},
  {key:"voucherRedemption",label:"Voucher Redemption 🎟️",sign:-1,hint:"Enter the amount as a normal number — we'll treat it as a deduction automatically"},
  {key:"voucherPurchase",label:"Voucher Purchase 🎫",sign:1,hasCashCard:true},
];
...
.btn.outline{background:none;border:2px solid #1A2744;color:#1A2744;}
...
/* Takings message */
.taking-msg{background:linear-gradient(135deg,#D1FAE5,#A7F3D0);border:1.5px solid #50DC78;border-radius:13px;padding:12px 14px;margin-bottom:10px;}
...
.taking-msg-new{background:linear-gradient(135deg,#FFF8EC,#FEF3C7);border:1.5px solid #F5A623;}
.taking-msg-new .taking-msg-head{color:#78350F;}
.taking-msg-new .taking-msg-detail{color:#92400E;}
/* Log entry */
...
/* PWA install banner */
.install-banner{background:#1A2744;color:#fff;padding:12px 16px;display:flex;justify-content:space-between;align-items:center;}
.install-text{font-size:13px;font-weight:600;}
.install-btn{background:#F5A623;color:#1A2744;border:none;border-radius:8px;padding:7px 14px;font-size:12px;font-weight:800;cursor:pointer;}
.install-close{background:none;border:none;color:rgba(255,255,255,.5);font-size:18px;cursor:pointer;margin-left:8px;}
/* GS Setup */
.gs-setup{background:#FFF8EC;border:2px solid #F5A623;border-radius:13px;padding:13px;margin-bottom:12px;}
.gs-title{font-size:13px;font-weight:800;color:#1A2744;margin-bottom:5px;}
`;
...
async function pushSheet(apiKey, spreadsheetId, sheetName, rows) {
  ...
  try {
    ...
    const res = await fetch(
      `https://sheets.googleapis.com/v4/spreadsheets/${spreadsheetId}/values/${encodeURIComponent(sheetName + "!A1")}?valueInputOption=USER_ENTERED&key=${apiKey}`,
...
function buildTSV(rows) { return rows.map(r => r.map(c => String(c ?? "")).join("\t")).join("\n"); }
function copyTSV(rows, t) {
  navigator.clipboard.writeText(buildTSV(rows))
    .then(() => t("📋 Copied! Open Google Sheets → click A1 → Ctrl+V (or ⌘+V)"))
    .catch(() => t("❌ Copy failed — try a different browser"));
}
...
function InstallBanner() {
  ...
  return (
    <div className="install-banner">
      <span className="install-text">📱 Add this app to your home screen</span>
      <button className="install-btn" onClick={() => { prompt?.prompt(); setShow(false); }}>Install</button>
  ...
  if (step === "code") return (
    <div className="auth-screen">
      ...
      <input className="inp code" type="password" inputMode="numeric" maxLength={8}
        placeholder="••••••••" value={code} onChange={e => { setCode(e.target.value); setErr(""); }} autoFocus />
...
function StaffRegister({ onBack, onRegister }) {
  const [name, setName] = useState(""); const [code, setCode] = useState(""); const [confirm, setConfirm] = useState(""); const [err, setErr] = useState("");
  function handle() {
  ...
  return (
    <div className="auth-screen">
      ...
      {err && <div className="err">{err}</div>}
      <button className="btn" onClick={handle}>Create Account</button>
    </div>
  );
}
...
// STAFF APP
// ═══════════════════════════════════════════════════════════════════
function StaffApp({ user, onLogout, rota, absences, setAbsences, clockLogs, setClockLogs, rejections, setRejections, confirmations, setConfirmations, takingsAssignment, takingsSubmissions, setTakingsSubmissions }) {
  ...
  const assigned = takingsAssignment?.staffId === user.id && takingsAssignment?.date === todayISO();
  const alreadySubmitted = takingsSubmissions.some(s => s.staffId === user.id && s.date === todayISO());
  const now = new Date();

  function t(m) { setToast(m); setTimeout(() => setToast(""), 2800); }

  function clockIn() {
    const time = nowTime(); setClockInTime(time); setClockedIn(true);
    setClockLogs(p => [...p, { id: Date.now(), staffId: user.id, name: user.name, date: todayISO(), in: time, out: null, note: "" }]);
    t("✅ Clocked in at " + time);
  }
  function clockOut() {
    const time = nowTime(); setClockedIn(false);
    setClockLogs(p => { const idx = [...p].reverse().findIndex(l => l.staffId === user.id && !l.out); if (idx < 0) return p; const arr = [...p]; arr[arr.length - 1 - idx] = { ...arr[arr.length - 1 - idx], out: time }; return arr; });
    t("👋 Clocked out at " + time);
  }

  const myRota = rota[user.id] || DAYS.map(() => ({ type: "Off", customIn: "", customOut: "" }));
  const myLogs = clockLogs.filter(l => l.staffId === user.id).sort((a, b) => b.date.localeCompare(a.date)).slice(0, 5);
  ...
  function submitTakings() {
    setTakingsSubmissions(p => [...p, { id: Date.now(), staffId: user.id, staffName: user.name, date: todayISO(), values: { ...takingsValues }, payTypes: { ...takingsPayTypes }, note: takingsNote, isNew: true }]);
    setTakingsValues({}); setTakingsPayTypes({}); setTakingsNote("");
    t("📊 Takings submitted to manager!");
    setTab("home");
  }
      ...
      {tab === "home" && (
        <div className="body">
              ...
              <div className="clk-hist">
                {myLogs.slice(0, 3).map(l => (
                  <div key={l.id} className="clk-row">
                    ...
                    <span>{l.in} → {l.out || "active"}</span>
                    <span style={{ fontWeight: 700 }}>{l.out ? parseHours(l.in, l.out).toFixed(1) + "h" : ""}</span>
                  </div>
                ))}
              </div>
            )}
          ...
          {myRota.map((shift, i) => {
            ...
            const rejected = rejections.some(r => r.staffId === user.id && r.day === DAYS[i]);
            const confirmed = confirmations.some(r => r.staffId === user.id && r.day === DAYS[i]);
              ...
              <div key={i} className={`rday${isToday ? " today" : ""}${isOff ? " off" : ""}`}>
                ...
                {!isOff && !rejected && !confirmed && (
                  <div className="rday-actions">
                    <button className="confirm-btn" onClick={() => { setConfirmations(p => [...p, { id: Date.now(), staffId: user.id, name: user.name, day: DAYS[i] }]); t("✅ Shift confirmed!"); }}>✓ OK</button>
          ...
          {myRota.map((shift, i) => {
            ...
            return (
              <div key={i} className={`rday${isToday ? " today" : ""}${isOff ? " off" : ""}`}>
                ...
                {!isOff && !rejected && !confirmed && (
                  <div className="rday-actions">
                    <button className="confirm-btn" onClick={() => { setConfirmations(p => [...p, { id: Date.now(), staffId: user.id, name: user.name, day: DAYS[i] }]); t("✅ Confirmed!"); }}>✓ OK</button>
                    <button className="reject-btn" onClick={() => { setRejectModal(i); setRejectReason(""); }}>✕ Can't</button>
                  </div>
                )}
                {confirmed && <span className="chip g">✓ Confirmed</span>}
        ...
        <div className="body">
          ...
          <div className="abs-card">
            ...
            <button className="btn" style={{ marginTop: 10 }} onClick={() => {
              ...
              setAbsences(p => [...p, { id: Date.now(), staffId: user.id, name: user.name, date: absDate, period: absPeriod }]);
              setAbsDate(""); setAbsPeriod(""); t("📅 Absence sent to manager!");
            }} disabled={!absDate || !absPeriod}>Send to Manager</button>
          </div>
          {myAbsences.length > 0 && (
            <>
          ...
          {alreadySubmitted ? (
            ...
            <>
              <div style={{ fontSize: 12, color: "#888", marginBottom: 14 }}>
                ...
                <strong>Enter all amounts as positive numbers</strong> — deductions like voucher redemptions are handled automatically.
              </div>
              {TAKING_FIELDS.map(f => (
                <div key={f.key} className="take-field">
                  ...
                  <input className="inp sm" style={{ display: "block", width: "100%", marginTop: 4 }} type="number" placeholder="0.00" min="0" value={takingsValues[f.key] || ""} onChange={e => setTakingsValues(p => ({ ...p, [f.key]: e.target.value }))} />
                </div>
              ))}
      ...
      {rejectModal !== null && (
        <div className="overlay" onClick={() => setRejectModal(null)}>
          <div className="sheet" onClick={e => e.stopPropagation()}>
            ...
            <textarea className="log-note" rows={3} placeholder="e.g. Doctor appointment, family commitment…" value={rejectReason} onChange={e => setRejectReason(e.target.value)} />
            <button className="btn danger" style={{ marginTop: 12 }} onClick={() => {
              setRejections(p => [...p, { id: Date.now(), staffId: user.id, name: user.name, day: DAYS[rejectModal], reason: rejectReason }]);
              setRejectModal(null); t("Rejection sent to manager");
...
// MANAGER APP
// ═══════════════════════════════════════════════════════════════════
function ManagerApp({ onLogout, staff, setStaff, rota, setRota, absences, clockLogs, setClockLogs, rejections, setRejections, confirmations, takingsSubmissions, setTakingsSubmissions, takingsAssignment, setTakingsAssignment, expenses, setExpenses, gsConfig, setGsConfig }) {
  const [tab, setTab] = useState("rota");
  const [toast, setToast] = useState("");
  const [kitchenStaff, setKitchenStaff] = useState([{ id: "k1", name: "Chef Wang", hours: "", rate: "", cashCard: "cash" }]);
  ...
  const newTakingsCount = takingsSubmissions.filter(s => s.isNew).length;

  function t(m) { setToast(m); setTimeout(() => setToast(""), 3000); }

  // ── Rota ──
  function getShift(sId, di) {
    return (rota[sId] || DAYS.map(() => ({ type: "Off", customIn: "", customOut: "" })))[di] || { type: "Off", customIn: "", customOut: "" };
  }
  function setShift(sId, di, field, val) {
    setRota(p => {
  ...
  function getAbsenceConflicts(staffId) {
    ...
    return staffAbsences.filter(a => {
      ...
      const shift = myRota[dayOfWeek];
      if (!shift || shift.type === "Off") return false;
      if (a.period === "Full Day") return true;
      if (a.period === "Morning" && shift.type === "Full Day (11am–close)") return true;
      if (a.period === "Evening" && (shift.type === "Night (5:30pm–close)" || shift.type === "Full Day (11am–close)")) return true;
      return false;
    });
  }
  ...
  function setExtras(sId, fn) { setPayrollExtras(p => ({ ...p, [sId]: fn(getExtras(sId)) })); }

  function calcStaffPay(s) {
    const myRota = rota[s.id] || [];
    const logsInRange = clockLogs.filter(l => l.staffId === s.id && l.date >= weekRange.start && l.date <= weekRange.end);
    let fullShifts = 0, nightShifts = 0, totalHrs = 0;
    myRota.forEach(sh => {
      if (!sh || sh.type === "Off") return;
      if (sh.type === "Full Day (11am–close)") fullShifts++;
      else if (sh.type === "Night (5:30pm–close)") nightShifts++;
    });
    logsInRange.forEach(l => { totalHrs += parseHours(l.in, l.out); });
    ...
    const total = base + tips + addTotal - dedTotal;
    return { fullShifts, nightShifts, totalHrs: totalHrs.toFixed(2), base: base.toFixed(2), tips: tips.toFixed(2), addTotal: addTotal.toFixed(2), dedTotal: dedTotal.toFixed(2), total: Math.max(0, total).toFixed(2) };
  }
  ...
  function buildPayrollRows() {
    ...
    staff.forEach(s => {
      ...
      const cc = s.cashCard || "cash";
      const extras = getExtras(s.id);
      const notesStr = (extras.notes || []).join("; ");
      rows.push([fmtRange(weekRange.start, weekRange.end), s.name, p.fullShifts, p.nightShifts, p.totalHrs, cc === "cash" ? p.total : cc === "mixed" ? (s.cashSplit || "0") : "0", cc === "card" ? p.total : cc === "mixed" ? (s.cardSplit || "0") : "0", p.tips, p.addTotal, p.dedTotal, p.total, notesStr]);
    });
    return rows;
  }

  function buildDailyRows() {
    const allDates = [...new Set([...takingsSubmissions.map(s => s.date), ...expenses.map(e => e.date)])].sort();
    ...
    allDates.forEach(date => {
      const sub = takingsSubmissions.find(s => s.date === date);
      const v = sub?.values || {};
      const dayExp = expenses.filter(e => e.date === date);
      const total = TAKING_FIELDS.reduce((s, f) => s + parseFloat(v[f.key] || 0) * f.sign, 0);
      const cashExp = dayExp.filter(e => e.payType === "cash").reduce((s, e) => s + e.amount, 0);
      const expNotes = dayExp.map(e => `${e.desc}(£${e.amount.toFixed(2)},${e.payType})`).join("; ");
      rows.push([fmtDate(date), v.deliveroo || 0, v.uber || 0, v.cash || 0, v.card || 0, v.online || 0, v.depositReceipt || 0, v.voucherRedemption || 0, v.voucherPurchase || 0, total.toFixed(2), (parseFloat(v.cash || 0) - cashExp).toFixed(2), (total - cashExp).toFixed(2), expNotes]);
    });
    return rows;
  }
  ...
  function buildWeeklyRows() {
    ...
    Object.entries(weekMap).sort().forEach(([ws, dates]) => {
      const { end } = getWeekRange(ws);
      let tot = {}; TAKING_FIELDS.forEach(f => tot[f.key] = 0); let cashExpTot = 0;
      dates.forEach(d => {
        const sub = takingsSubmissions.find(s => s.date === d);
        if (sub) TAKING_FIELDS.forEach(f => { tot[f.key] += parseFloat(sub.values?.[f.key] || 0); });
        cashExpTot += expenses.filter(e => e.date === d && e.payType === "cash").reduce((a, e) => a + e.amount, 0);
      });
      const total = TAKING_FIELDS.reduce((s, f) => s + tot[f.key] * f.sign, 0);
      rows.push([fmtRange(ws, end), tot.deliveroo.toFixed(2), tot.uber.toFixed(2), tot.cash.toFixed(2), tot.card.toFixed(2), tot.online.toFixed(2), tot.depositReceipt.toFixed(2), tot.voucherRedemption.toFixed(2), tot.voucherPurchase.toFixed(2), total.toFixed(2), (tot.cash - cashExpTot).toFixed(2), (total - cashExpTot).toFixed(2)]);
    });
    return rows;
  }
  ...
  async function doPayrollExport() {
    if (gsConfig.payrollId && gsConfig.apiKey) {
      t("⏳ Pushing payroll to Google Sheets…");
      const r = await pushSheet(gsConfig.apiKey, gsConfig.payrollId, "Payroll", buildPayrollRows());
      t(r.ok ? "✅ Payroll sheet updated!" : "❌ " + r.err);
    } else {
      copyTSV(buildPayrollRows(), t);
    }
  }
  ...
  async function doTakingsExport() {
    if (gsConfig.takingsId && gsConfig.apiKey) {
      ...
      const r1 = await pushSheet(gsConfig.apiKey, gsConfig.takingsId, "Daily", buildDailyRows());
      if (!r1.ok) { t("❌ " + r1.err); return; }
      t("⏳ Updating Weekly tab…");
      const r2 = await pushSheet(gsConfig.apiKey, gsConfig.takingsId, "Weekly", buildWeeklyRows());
      t(r2.ok ? "✅ Takings sheet updated (Daily + Weekly)!" : "❌ Weekly: " + r2.err);
    } else {
      const all = [...buildDailyRows(), [""], [""], ...buildWeeklyRows()];
      copyTSV(all, t);
    }
  }

  // ── Add/Deduct inline component ──
  function AddDeductRow({ sId, type }) {
    const extras = getExtras(sId);
    const items = type === "add" ? (extras.additions || []) : (extras.deductions || []);
    ...
    return (
      <div>
        ...
        <div className="add-row">
          <select className="add-inp" style={{ flex: "none", width: "auto", padding: "6px 7px", fontSize: 11 }} value={label} onChange={e => setLabel(e.target.value)}>
            {labels.map(l => <option key={l}>{l}</option>)}
          </select>
          <input className="add-inp" type="number" placeholder="£0" min="0" value={amount} onChange={e => setAmount(e.target.value)} style={{ width: 62 }} />
          <button className={`add-btn-sm${type === "ded" ? " r" : ""}`} onClick={() => {
            if (!amount) return;
            const fl = label === "Other" && custom ? custom : label;
            setExtras(sId, ex => ({ ...ex, [key]: [...(ex[key] || []), { label: fl, amount }] }));
            setAmount(""); setCustom("");
          }}>+ Add</button>
        </div>
  ...
  function StaffSetupModal({ s, onClose }) {
    ...
    const [cashCard, setCC] = useState(s.cashCard || "cash");
    return (
      <div className="overlay" onClick={onClose}>
        <div className="sheet" onClick={e => e.stopPropagation()}>
          <div className="sheet-title">⚙️ Pay Settings — {s.name}</div>
          <div className="sheet-sub">These settings are private</div>
          ...
          <button className="btn" onClick={() => { setStaff(p => p.map(x => x.id === s.id ? { ...x, payType, rate, shiftRate, nightRate, cashCard } : x)); t(`✅ ${s.name} pay settings saved`); onClose(); }}>Save Settings</button>
  ...
  function GsModal({ onClose }) {
    const [apiKey, setApiKey] = useState(gsConfig.apiKey || "");
    ...
    return (
      <div className="overlay" onClick={onClose}>
        <div className="sheet" onClick={e => e.stopPropagation()}>
          ...
          <div className="sheet-sub">Connect two spreadsheets: one for payroll, one for takings</div>
          <label className="lbl">Google API Key (shared)</label>
          ...
          <input className="inp sm" style={{ display: "block", width: "100%", marginBottom: 6 }} placeholder="1BxiMV… (needs a 'Payroll' tab)" value={payrollId} onChange={e => setPayrollId(e.target.value)} />
          <label className="lbl">📊 Takings Spreadsheet ID</label>
          <input className="inp sm" style={{ display: "block", width: "100%", marginBottom: 14 }} placeholder="1CyiNW… (needs 'Daily' and 'Weekly' tabs)" value={takingsId} onChange={e => setTakingsId(e.target.value)} />
          <div style={{ background: "#F7F4EF", borderRadius: 10, padding: "10px 12px", fontSize: 12, color: "#888", marginBottom: 14, lineHeight: 1.7 }}>
            <strong>Setup steps:</strong><br />
            1. Create two Google Sheets<br />
            2. In the Payroll sheet: add a tab named <strong>Payroll</strong><br />
            3. In the Takings sheet: add tabs named <strong>Daily</strong> and <strong>Weekly</strong><br />
            4. Get your API key from Google Cloud Console (enable Sheets API)<br />
            5. Share both sheets with <em>Anyone with the link can edit</em> or add your service account
          </div>
          <button className="btn" onClick={() => { setGsConfig({ apiKey, payrollId, takingsId }); t("✅ Sheets config saved!"); onClose(); }}>Save Config</button>
          <button className="btn sec" onClick={onClose}>Cancel</button>
        </div>
      </div>
    );
    ...
    <div className="app">
      ...
      <div className="mgr-tabs">
        {[
          ...
          { id: "takings", label: `📊 Takings${newTakingsCount > 0 ? ` (${newTakingsCount})` : ""}` },
      ...
      <div className="mgr-body">
              ...
              <div style={{ marginBottom: 10 }}>
                ...
                {rejections.map(r => (
                  <div key={r.id} className="rej-banner">
                    <span><strong>{r.name}</strong> can't do <strong>{r.day}</strong>{r.reason ? ` — "${r.reason}"` : ""}</span>
                    <button onClick={() => setRejections(p => p.filter(x => x.id !== r.id))} style={{ background: "none", border: "none", cursor: "pointer", fontSize: 16 }}>✓</button>
                  </div>
                ))}
              </div>
            )}
                ...
                <div key={s.id} className="card">
                  <div className="card-head">
                    <div>
                      ...
                      <div className="card-sub">{s.payType === "shift" ? `Full £${s.shiftRate} / Night £${s.nightRate}` : `£${s.rate}/hr`}</div>
                    </div>
                  ...
                  {conflicts.length > 0 && (
                    <div className="warn-banner">
                      ...
                      <div className="warn-sub">{s.name} has reported absence on: {conflicts.map(c => `${isoToDisplay(c.date)} (${c.period})`).join(", ")}</div>
                    </div>
                  )}
                  {DAYS.map((d, i) => {
                    const sh = getShift(s.id, i);
                    return (
                      <div key={d} style={{ display: "flex", alignItems: "center", gap: 5, marginBottom: 5 }}>
                        ...
                        <select className="inp sm" style={{ flex: 1 }} value={sh.type || "Off"} onChange={e => setShift(s.id, i, "type", e.target.value)}>
                          {SHIFT_TYPES.map(o => <option key={o}>{o}</option>)}
                        </select>
                        {sh.type === "Custom" && (
                          <>
                            <input type="time" className="inp time" value={sh.customIn || ""} onChange={e => setShift(s.id, i, "customIn", e.target.value)} />
                            <span style={{ fontSize: 10, color: "#aaa" }}>–</span>
                            <input type="time" className="inp time" value={sh.customOut || ""} onChange={e => setShift(s.id, i, "customOut", e.target.value)} />
                          </>
                        )}
                      </div>
                    );
                  })}
                  <button className="btn green" style={{ marginTop: 8, padding: "11px" }} onClick={() => t(`✅ Rota sent to ${s.name}!`)}>
        ...
        {tab === "clock" && (
          <>
            ...
            {staff.map(s => {
              const logs = clockLogs.filter(l => l.staffId === s.id).sort((a, b) => b.date.localeCompare(a.date));
              const totalHrs = logs.reduce((a, l) => a + parseHours(l.in, l.out), 0);
              return (
                <div key={s.id} className="card">
                  <div className="card-name">👤 {s.name}</div>
                  <div className="card-sub" style={{ marginBottom: 10 }}>Total: {totalHrs.toFixed(1)} hrs</div>
                  {logs.length === 0 && <div style={{ fontSize: 12, color: "#ccc", fontStyle: "italic" }}>No clock records yet</div>}
                  {logs.map(l => (
                    <div key={l.id} className="log-entry">
                      <div className="log-top">
                        <span style={{ fontSize: 13, fontWeight: 700, color: "#1A2744" }}>{isoToDisplay(l.date)}</span>
                        <span style={{ fontSize: 13, fontWeight: 800, color: l.out ? "#1A2744" : "#50DC78" }}>{l.out ? parseHours(l.in, l.out).toFixed(1) + "h" : "active"}</span>
                      </div>
                      <div className="log-edit-row">
                        <span className="log-edit-lbl">In</span>
                        <input type="time" className="inp time" value={l.in || ""} onChange={e => setClockLogs(p => p.map(x => x.id === l.id ? { ...x, in: e.target.value } : x))} />
                        <span className="log-edit-lbl">Out</span>
                        <input type="time" className="inp time" value={l.out || ""} onChange={e => setClockLogs(p => p.map(x => x.id === l.id ? { ...x, out: e.target.value } : x))} />
                      </div>
                      <textarea className="log-note" rows={2} placeholder="Note / question about these hours…" value={l.note || ""} onChange={e => setClockLogs(p => p.map(x => x.id === l.id ? { ...x, note: e.target.value } : x))} />
                    </div>
                  ))}
                  <button className="btn sm" style={{ marginTop: 9, background: "#F5A623" }} onClick={() => setClockLogs(p => [...p, { id: Date.now(), staffId: s.id, name: s.name, date: todayISO(), in: "", out: "", note: "" }])}>+ Add Entry</button>
                </div>
              );
            })}
          </>
        ...
        {tab === "payroll" && (
          <>
                ...
                <div key={s.id} className="pay-card">
                  <div className="pay-head">
                    <div>
                      ...
                      <span className="chip" style={{ background: cc === "cash" ? "#D1FAE5" : cc === "card" ? "#DBEAFE" : "#FEF3C7", color: cc === "cash" ? "#065F46" : cc === "card" ? "#1E40AF" : "#78350F" }}>
                        {cc === "cash" ? "💵 Cash" : cc === "card" ? "💳 Card" : "Split"}
                      </span>
                    </div>
                  ...
                  <div className="pay-body">
                    {s.payType === "shift" ? (
                      <>
                        <div className="row"><span>Full Day shifts</span><span className="row-bold">{p.fullShifts} × £{s.shiftRate} = £{(p.fullShifts * parseFloat(s.shiftRate || 0)).toFixed(2)}</span></div>
                        <div className="row"><span>Night shifts</span><span className="row-bold">{p.nightShifts} × £{s.nightRate} = £{(p.nightShifts * parseFloat(s.nightRate || 0)).toFixed(2)}</span></div>
                      </>
                    ) : (
                      <div className="row"><span>Hours</span><span className="row-bold">{p.totalHrs} h × £{s.rate} = £{p.base}</span></div>
                    )}
                    ...
                    <div style={{ marginTop: 8 }}>
                      ...
                      {(extras.notes || []).map((n, i) => (
                        <div key={i} style={{ display: "flex", justifyContent: "space-between", padding: "2px 0" }}>
                          ...
                          <button onClick={() => setExtras(s.id, ex => ({ ...ex, notes: ex.notes.filter((_, j) => j !== i) }))} style={{ background: "none", border: "none", cursor: "pointer", fontSize: 12, color: "#ccc" }}>✕</button>
                        </div>
                      ))}
                      <div className="add-row" style={{ marginTop: 5 }}>
                        <select className="add-inp" style={{ fontSize: 11, padding: "5px 7px" }} id={`ns-${s.id}`}>
                        ...
                        <button className="add-btn-sm" onClick={() => {
                          ...
                          if (sel.value === "Custom") {
                            const cn = window.prompt("Enter custom note:");
                            if (cn) setExtras(s.id, ex => ({ ...ex, notes: [...(ex.notes || []), cn] }));
                          } else {
                            setExtras(s.id, ex => ({ ...ex, notes: [...(ex.notes || []), sel.value] }));
                          }
                        }}>+ Note</button>
                      </div>
                    </div>
                    {cc === "mixed" && (
                      <div style={{ marginTop: 10, background: "#F7F4EF", borderRadius: 9, padding: "9px" }}>
                        <div style={{ fontSize: 11, fontWeight: 700, color: "#888", marginBottom: 6 }}>SPLIT PAYMENT (total £{p.total})</div>
                        <div style={{ display: "flex", gap: 8 }}>
                          <div style={{ flex: 1 }}><div style={{ fontSize: 10, color: "#aaa", marginBottom: 3 }}>💵 Cash (£)</div><input type="number" className="inp sm" style={{ width: "100%" }} placeholder="0.00" value={s.cashSplit || ""} onChange={e => setStaff(pp => pp.map(x => x.id === s.id ? { ...x, cashSplit: e.target.value } : x))} /></div>
                          <div style={{ flex: 1 }}><div style={{ fontSize: 10, color: "#aaa", marginBottom: 3 }}>💳 Card (£)</div><input type="number" className="inp sm" style={{ width: "100%" }} placeholder="0.00" value={s.cardSplit || ""} onChange={e => setStaff(pp => pp.map(x => x.id === s.id ? { ...x, cardSplit: e.target.value } : x))} /></div>
                        </div>
                      </div>
                    )}
            ...
            <div style={{ display: "flex", gap: 6, marginBottom: 10 }}>
              ...
              <button className="btn sm navy" onClick={() => { if (!newKName.trim()) return; setKitchenStaff(p => [...p, { id: Date.now(), name: newKName.trim(), hours: "", rate: "", cashCard: "cash" }]); setNewKName(""); }}>Add</button>
            </div>
              ...
              return (
                <div key={k.id} className="card white">
                  <div className="card-head">
                    ...
                    <button onClick={() => setKitchenStaff(p => p.filter((_, j) => j !== i))} style={{ background: "none", border: "none", cursor: "pointer", fontSize: 15, color: "#ddd" }}>🗑️</button>
                  </div>
                  <div style={{ display: "flex", gap: 7, marginBottom: 8, flexWrap: "wrap" }}>
                    <div style={{ flex: 1, minWidth: 76 }}><div style={{ fontSize: 10, fontWeight: 700, color: "#aaa", marginBottom: 3 }}>HOURS</div><input type="number" min="0" className="inp sm" style={{ width: "100%" }} placeholder="0" value={k.hours} onChange={e => setKitchenStaff(p => p.map((x, j) => j === i ? { ...x, hours: e.target.value } : x))} /></div>
                    <div style={{ flex: 1, minWidth: 76 }}><div style={{ fontSize: 10, fontWeight: 700, color: "#aaa", marginBottom: 3 }}>£/HR</div><input type="number" min="0" className="inp sm" style={{ width: "100%" }} placeholder="0.00" value={k.rate} onChange={e => setKitchenStaff(p => p.map((x, j) => j === i ? { ...x, rate: e.target.value } : x))} /></div>
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    ...
                    <div className="toggle">
                      {["cash", "card"].map(c => <button key={c} className={`tgl-btn${k.cashCard === c ? " active" : ""}`} onClick={() => setKitchenStaff(p => p.map((x, j) => j === i ? { ...x, cashCard: c } : x))}>{c === "cash" ? "💵" : "💳"} {c}</button>)}
                    </div>
                  </div>
                </div>
              );
            ...
            <div className="exp-sec">
              ...
              <button className="exp-btn primary" onClick={doPayrollExport}>
                {gsConfig.payrollId && gsConfig.apiKey ? "🔗 Push to Payroll Sheet" : "📋 Copy — Paste into Payroll Sheet"}
              </button>
        ...
        {tab === "takings" && (
          <>
            ...
            <div className="card">
              ...
              <select className="inp sm" style={{ display: "block", width: "100%", marginBottom: 8 }} value={takingsAssignment?.staffId || ""}
                onChange={e => setTakingsAssignment(e.target.value ? { staffId: e.target.value, date: todayISO() } : null)}>
                <option value="">— Not assigned (manager will record) —</option>
                {staff.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}
              </select>
              {takingsAssignment?.staffId && <span className="chip g">✓ Assigned for {isoToDisplay(takingsAssignment.date)}</span>}
            </div>

            {/* Submissions from staff — shown as messages */}
            {takingsSubmissions.length > 0 && (
              <>
                <div style={{ fontSize: 13, fontWeight: 800, color: "#1A2744", marginBottom: 8 }}>📨 Submissions</div>
                {[...takingsSubmissions].sort((a, b) => b.date.localeCompare(a.date)).map(sub => {
                  const total = TAKING_FIELDS.reduce((s, f) => s + parseFloat(sub.values?.[f.key] || 0) * f.sign, 0);
                  return (
                    <div key={sub.id} className={`taking-msg${sub.isNew ? " taking-msg-new" : ""}`}>
                      <div className="taking-msg-head">
                        {sub.isNew ? "🆕 New — " : "✓ "}{sub.staffName || "Manager"} · {isoToDisplay(sub.date)}
                        <span style={{ float: "right", fontSize: 14, fontWeight: 900 }}>£{total.toFixed(2)}</span>
                      </div>
                      <div className="taking-msg-detail">
                        {TAKING_FIELDS.filter(f => parseFloat(sub.values?.[f.key] || 0) > 0).map(f => `${f.label.replace(/[🛵💵💳🌐🎟️🎫]/g,"").trim()}: £${sub.values[f.key]}`).join(" · ")}
                      </div>
                      {sub.note && <div style={{ marginTop: 4, fontSize: 12, opacity: .8 }}>📝 {sub.note}</div>}
                      {sub.isNew && (
                        <button className="btn sm" style={{ marginTop: 8, background: "#065F46", color: "#fff" }}
                          onClick={() => setTakingsSubmissions(p => p.map(x => x.id === sub.id ? { ...x, isNew: false } : x))}>Mark Seen ✓</button>
                      )}
                    </div>
                  );
                })}
            ...
            <div className="card" style={{ marginTop: 10 }}>
              ...
              <ManagerTakingsForm takingsSubmissions={takingsSubmissions} setTakingsSubmissions={setTakingsSubmissions} t={t} />
            </div>

            <div className="exp-sec">
              <div className="exp-title">📤 Export Takings (Daily + Weekly)</div>
              <button className="exp-btn primary" onClick={doTakingsExport}>
                {gsConfig.takingsId && gsConfig.apiKey ? "🔗 Push to Takings Sheet" : "📋 Copy — Paste into Takings Sheet"}
              </button>
              <div className="exp-hint">Open your Takings spreadsheet → click A1 → Ctrl+V. Daily data goes first, then the Weekly summary below.</div>
            </div>
          </>
        )}

        {/* ── EXPENSES ── */}
        {tab === "expenses" && <ExpensesTab expenses={expenses} setExpenses={setExpenses} t={t} />}
        ...
        {tab === "absence" && (
          <>
            ...
            {absences.length === 0
              ? <div className="empty"><div className="empty-icon">📅</div><div className="empty-text">No absences reported</div></div>
              : absences.map(a => (
                <div key={a.id} style={{ background: "#FFF8EC", border: "1.5px solid #F5A623", borderRadius: 12, padding: "10px 13px", marginBottom: 9 }}>
                  <div style={{ fontWeight: 800, color: "#1A2744", fontSize: 13 }}>👤 {a.name}</div>
                  <div style={{ fontSize: 12, color: "#888", marginTop: 2 }}>{isoToDisplay(a.date)} — {a.period}</div>
                </div>
              ))
            }
          </>
...
// ═══════════════════════════════════════════════════════════════════
function ManagerTakingsForm({ takingsSubmissions, setTakingsSubmissions, t }) {
  const [values, setValues] = useState({});
  const [payTypes, setPayTypes] = useState({});
  const [note, setNote] = useState("");
  const [date, setDate] = useState(todayISO());
  function submit() {
    setTakingsSubmissions(p => [...p, { id: Date.now(), staffId: "manager", staffName: "Manager", date, values: { ...values }, payTypes: { ...payTypes }, note, isNew: false }]);
    setValues({}); setNote(""); setDate(todayISO()); t("✅ Takings saved!");
  }
  return (
    <>
      ...
      {TAKING_FIELDS.map(f => (
        <div key={f.key} className="take-field">
          <div className="take-lbl">
            ...
            {f.hasCashCard && (
              <div className="toggle" style={{ transform: "scale(.8)", transformOrigin: "right" }}>
                {["cash", "card"].map(c => <button key={c} className={`tgl-btn${(payTypes[f.key] || "cash") === c ? " active" : ""}`} onClick={() => setPayTypes(p => ({ ...p, [f.key]: c }))}>{c}</button>)}
              </div>
            )}
          </div>
          {f.hint && <div className="take-hint">{f.hint}</div>}
      ...
      <button className="btn" onClick={submit}>Save Takings</button>
    </>
  );
}
...
// EXPENSES TAB
// ═══════════════════════════════════════════════════════════════════
function ExpensesTab({ expenses, setExpenses, t }) {
  const [desc, setDesc] = useState(""); const [amount, setAmount] = useState(""); const [payType, setPayType] = useState("cash"); const [date, setDate] = useState(todayISO());
  return (
    <>
      ...
      <div className="card">
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 10, alignItems: "flex-end" }}>
          ...
          <div>
            ...
            <div className="toggle">
              {["cash", "card"].map(c => <button key={c} className={`tgl-btn${payType === c ? " active" : ""}`} onClick={() => setPayType(c)}>{c === "cash" ? "💵" : "💳"} {c}</button>)}
            </div>
          </div>
        </div>
        <button className="btn" onClick={() => {
          if (!desc || !amount) return t("Fill in description and amount");
          setExpenses(p => [...p, { id: Date.now(), desc, amount: parseFloat(amount), payType, date }]);
          setDesc(""); setAmount(""); t("✓ Expense added");
        }}>Add Expense</button>
      </div>
      {expenses.length === 0
          ...
          <>
            ...
            {[...expenses].sort((a, b) => b.date.localeCompare(a.date)).map(e => (
              <div key={e.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "9px 0", borderBottom: "1px solid #F0F0F0" }}>
                <div>
                  <div style={{ fontSize: 13, fontWeight: 700, color: "#1A2744" }}>{e.desc}</div>
                  <div style={{ fontSize: 11, color: "#aaa" }}>{fmtDate(e.date)} · {e.payType === "cash" ? "💵 Cash" : "💳 Card"}</div>
                </div>
                <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                  <div style={{ fontSize: 13, fontWeight: 800, color: "#E05252" }}>-£{e.amount.toFixed(2)}</div>
                  <button onClick={() => setExpenses(p => p.filter(x => x.id !== e.id))} style={{ background: "none", border: "none", cursor: "pointer", fontSize: 14, color: "#ccc" }}>🗑️</button>
                </div>
              </div>
            ))}
            <div style={{ display: "flex", justifyContent: "space-between", padding: "10px 0 0", fontSize: 14, fontWeight: 800, color: "#1A2744", borderTop: "2px solid #F0F0F0", marginTop: 4 }}>
              <span>Total</span><span>£{expenses.reduce((a, e) => a + e.amount, 0).toFixed(2)}</span>
            </div>
          </>
        )}
    </>
...
export default function App() {
  ...
  const [takingsSubmissions, setTakingsSubmissions] = useState([]);
  ...
  return (
    <>
      ...
      <div className="app">
        ...
        {screen === "staffLogin" && <StaffLogin staff={staff} onLogin={u => { setUser(u); setScreen("staff"); }} onBack={() => setScreen("role")} onRegister={() => setScreen("staffRegister")} />}
        {screen === "staffRegister" && <StaffRegister onBack={() => setScreen("staffLogin")} onRegister={u => { setStaff(p => [...p, u]); setUser(u); setScreen("staff"); }} />}
        {screen === "managerLogin" && <ManagerLogin onLogin={() => setScreen("manager")} onBack={() => setScreen("role")} />}
        {screen === "staff" && user && (
          <StaffApp user={user} onLogout={() => { setUser(null); setScreen("role"); }}
            rota={rota} absences={absences} setAbsences={setAbsences}
            clockLogs={clockLogs} setClockLogs={setClockLogs}
            rejections={rejections} setRejections={setRejections}
            confirmations={confirmations} setConfirmations={setConfirmations}
            takingsAssignment={takingsAssignment}
            takingsSubmissions={takingsSubmissions} setTakingsSubmissions={setTakingsSubmissions}
          />
        )}
        {screen === "manager" && (
          <ManagerApp onLogout={() => setScreen("role")}
            ...
            takingsSubmissions={takingsSubmissions} setTakingsSubmissions={setTakingsSubmissions}
            takingsAssignment={takingsAssignment} setTakingsAssignment={setTakingsAssignment}
            expenses={expenses} setExpenses={setExpenses}
            gsConfig={gsConfig} setGsConfig={setGsConfig}
          />
        )}
      </div>
    </>
